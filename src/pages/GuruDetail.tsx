...
const GuruDetail = () => {
  ...
  // Find the specific guru
  const guru = guruCollections.find(g => g.id === guruId);
  ...
  // Fetch thumbnails for this guru's videos
  useEffect(() => {
    if (!guru) return;
    ...
    const fetchThumbnails = async () => {
      try {
        ...
        const videoIds = guru.videos.map(video => video.youtubeId).join(',');
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#1a1a2e] via-[#16213e] to-[#0f0f1e]">
        ...
        <div className="px-6 pt-8 pb-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-center flex-1">
              <h1 className="text-2xl font-bold text-white">{guru.name}</h1>
              <p className="text-xs text-white/60">{guru.description}</p>
            </div>
      ...
      <div className="pt-24">
        ...
        <div className="px-6 pb-24 pt-4">
          <h2 className="text-2xl font-bold text-white mb-4">Katha Videos</h2>
